...
contract StateRollbackBehaviorval {
...
/*
Title: State roolback behavior and CEI pattern in withdraw()

Severity: Medium

Reason: Updating storage before validation can be confusing and may introduce vulnerabilities when external interactions are added

Location: Contract: StateRoolbackBehavior
          Function: withdraw()

Vulnerability Description: The withdraw() function updates storage variables befor performing validation

Although solidity automatically rools back state changes when a require() fails, updating state before validation violates the Checks -> Effects -> Interactions (CEI) pattern

This can become dangerous when:
1. External contract calls are added
2. Ether transfers are performed
3. Reentrancy opportunities exist

Impact: An incorrect implementation may:
- Cause unexpected state changes
- Introduce reentrancy vulnerabilities
- Make contrack logic harder to audit

Proof of Concept:
Initial State
totalCounter = 100;
userCounter[Alice] = 100;
Alice Calls
withdraw(50);
Execution
...
All previous storage changes are rolled back.

Root Cause: State variables are modified before validation
...
Recommendation: Follow the CEL pattern:
1. Checks
2. Effects
3. Interactions
Validate everything before modifying state

Use custom errors instead of require strings for lower gas cousts
...
contract StateRollbackBehavior {
    ...
    */
    ...
    function safeWithdraw(uint256 _amount) external
    {
        ...
        if(userCounter[msg.sender] < _amount)
        {
