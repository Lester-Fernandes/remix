...
contract CalldataRestrictionval {
...
/*
Title: Safe modification of calldata string using memory copy

Severity: Low

Reason: calldata is immutable (read-only). Any attempt to modify calldata directly results in a compiler error. To safely modify
        input data, it must first be copied into memory.

Location: Contract: calldataRestriction
          New function: modifyString()

Vulnerability Description: The contract demonstrates that calldata arrays cannot be modified directly:
_numbers[0] = 999; 

the same rule applies to strings: 
string calldata _text

A calldata string is read-only.

To safely modify or transform the string:

1. Accept string in calldata
2. Copy it into memory
3. Create a modified version
4. Return the new string

Impact: Attempting to modify calldata directly
- causes compilation failure
- violates solidity's immutability rules
- prevents deployment
Using a memory copy allows safe manipulation

Proof of concept:
input: "Blockchain"
memory copy: string memory temp = _text;
modified output: "Hello Blockchain"
- Original calldata remains unchanged

Root Cause: calldata is designed as a read-only area for external inputs
            Direct modification is prohibited by the compiler

Recommendation: Copy calldata data into memory before modidication
    Example: string memory temp = _text;
    Then build and return a modified string
...
// CALLDATA STRING -> MEMORY STRING

    function modifyString(string calldata _text) external pure returns (string memory)
    {
        string memory temptext = _text;

        string memory modifiedText = string (abi.encodePacked("Hello ", temptext));
