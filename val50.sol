...
contract DebugExecutionTracingval {
...
/*
Title: Satle-state bug in vulnerableReward() Due to incorrect Excution order

Severity: Medium

Reason: The reward calculation uses an outdated balance value bacause the reward is calculated before the balance update occurs

Location: Contract: DebugExectionTracing
          Function: VulnerableReward()

Vulnerability Description: The function calculates the user's reward using the current balance
The function calculates the user's reward using the current balance:

bonuses[msg.sender] = balances[msg.sender] / 10;

Only after the reward is calculated does it update the balance:

balances[msg.sender] += _amount;

As a result, the reward is based on the old balance rather than the new balance.

Impact: 
- Incorrect reward calculations
- Users receive lower rewards than expected
- Accounting inconsistencies
- Logic errors in reward systems
...
Recommendation: Update the balance first and calculate rewards afterward

*/
contract DebugExecutionTracing { // DEPLOY CONTRACT
...
// Current funds in ystem
    uint256 public totalSystemBalance;
    ...
    */
        ...
        external
    {
        ...
        */
        ...
        bonuses[msg.sender] = balances[msg.sender] / 10; // recalculate bonus using the new balance
...
// Reward uses OLD balance
    ...
    function vulnerableReward( // TRACE VULNERABLEREWARD
        uint256 _amount
    )
        external
    {
...
// SAFE REWARD FUNCTION
    function safeReward(uint256 _amount) external 
    {
        balances[msg.sender] += _amount;
